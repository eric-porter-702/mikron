$(function() {
    ...
    var input, a, b, c, ip, a1, a2, done, runfile, toRun, mode, errorString;
    var list, loop, stringList, numberList, charList;
    var wrong, maxLoop, compareTo, nest, funIndex;
    var cmdStore = [10, 16, 100, 256];
    var replies = [
        "what are you doing",
		"stop it",
    ...
    var d = v => v !== undefined;
    var tS = v => v.toString();
    list = loop = stringList = numberList = charList = [];
    ...
    function runProgram(program, debug, arg1, arg2) {
        try {
            ...
            function diyad(a, b, c) {
                ...
                numberList = program.match(/\d+/g);
                numberList = numberList === null ? numberList : numberList.map(z => p(z)).reverse();
                if (program[ip - 1] == '"' && d(program[ip - 1])) a = stringList.pop();
                else if (program[ip - 2] == '`' && program[ip - 1] != '"' && d(program[ip - 1])) a = charList.pop();
